...
var chat = new function(){
	...
	function sendChat(str){
		dispatchChatEvent(str, "Visitor"); //dispatches event when user types some query

		gsap.fromTo('.typing', .1, {display: 'none', x: 10}, {display: 'block', x: 0, delay: 1});

		// this is for responding to the users query
	...
	function dispatchChatEvent(msg, from){
		...
		document.querySelector('#chatSubmit').dispatchEvent(event);

		gsap.fromTo('.typing', 0, {display: 'block'}, {display: 'none'});
